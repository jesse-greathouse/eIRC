...
my @systemDependencies = qw(
    base-devel intltool autoconf automake gcc curl pkgconf cpanminus
...
sub install_system_dependencies {
    ...
    print "Pacman is required for updating and installing system dependencies.\n";

    # Sync package list and update system
    my @updateCmd = ('pacman', '-Sy');
    system(@updateCmd);
    ...
    foreach my $pkg (@systemDependencies) {
        my $check = system("pacman -Qi $pkg > /dev/null 2>&1");
        if ($check != 0) {
            push @to_install, $pkg;
        } else {
    ...
    if (@to_install) {
        my @installCmd = ('pacman', '--noconfirm', '-S', @to_install);
        system(@installCmd);

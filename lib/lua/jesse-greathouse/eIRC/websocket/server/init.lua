...
--]]
function _M.run(nick, realname, server_addr, port, channels, instance_id, sasl_secret)
    ...
    local ok, err = irc.start_client(
        nick, realname, server_addr, port, channels, instance_id, sasl_secret
    )
    if not ok then
